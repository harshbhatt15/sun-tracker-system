...
export function SolarTrackerVisualization({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {!s.night && (
        ...
        >
          <div className="relative grid place-items-center">
            ...
            <span className="sun-glow relative block h-10 w-10 rounded-full animate-spin-slow" style={{ backgroundImage: "var(--gradient-solar)" }}>
              <span className="absolute inset-0 rounded-full" style={{ boxShadow: "0 0 0 4px oklch(0.88 0.16 88 / 0.35)" }} />
            </span>
          </div>
        </div>
      )}
      ...
      {/* tracker hardware */}
      <div className="absolute bottom-[8%] left-1/2 -translate-x-1/2" style={{ perspective: "900px" }}>
        <div
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              backgroundImage:
                "linear-gradient(150deg, oklch(0.5 0.13 258), oklch(0.3 0.1 262))",
          ...
          <div className="relative -mt-1 grid place-items-center">
            <span
              className={cn(
                ...
                s.motors.elevation !== "IDLE" && s.motors.elevation !== "STOPPED" && "animate-spin-slow",
              )}
            />
          </div>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 flex flex-wrap items-start justify-between gap-2 p-3 text-xs">
        <div className="rounded-lg bg-background/70 px-3 py-2 font-mono backdrop-blur">
          ...
          <div>AZ {s.sun.azimuth.toFixed(0)}° · EL {Math.max(0, s.sun.elevation).toFixed(0)}°</div>
        ...
        <div className="rounded-lg bg-background/70 px-3 py-2 text-right font-mono backdrop-blur">
          ...
          <div>AZ {s.panel.azimuth.toFixed(0)}° · EL {s.panel.elevation.toFixed(0)}°</div>
