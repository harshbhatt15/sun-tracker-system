...
export const Route = createFileRoute("/sensors")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Live LDR sensor readings, sensor health and light-difference analytics for the solar tracker.",
      },
      { property: "og:title", content: "Sensor Data — SunTrack Pro" },
      { property: "og:description", content: "Four-LDR array readings with animated gauges and live charts." },
    ],
  }),
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {sensorMeta.map((m) => (
          <Panel key={m.key} title={m.label} icon={<GaugeIcon className="h-4 w-4" />}>
            <div className="font-display text-4xl font-bold tabular-nums" style={{ color: m.color }}>
              {s.sensors[m.key]}
            </div>
            <div className="mt-3">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
        <Panel title="Analytics">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Metric label="Difference" value={diff} tone={diff > s.settings.deadband ? "danger" : "success"} />
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            <StatusBadge tone={health}>
              Sensor health: {health === "success" ? "good" : health === "warning" ? "imbalanced" : "fault"}
            </StatusBadge>
