  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "SunTrack Pro — Solar Tracking Dashboard" },
      {
        ...
        content:
          "Sun position, panel orientation, LDR sensors, motors and energy yield in real time.",
      },
    ],
  }),
  component: Dashboard,
    ...
    <div className="space-y-4">
      ...
      <section className="card-surface relative overflow-hidden p-5 sm:p-7">
        ...
        <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
          An automated dual-axis solar tracking system designed to continuously orient a solar panel
          toward the Sun for improved solar energy capture.
        </p>
        <div className="mt-5 grid grid-cols-2 gap-2 lg:grid-cols-4">
          <Metric
            label="Tracking efficiency"
            value={s.efficiency.toFixed(1)}
            unit="%"
            tone="solar"
          />
          ...
          <Metric
            label="Panel angle"
            value={`${s.panel.azimuth.toFixed(0)}° / ${s.panel.elevation.toFixed(0)}°`}
          />
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Button
            ...
            variant="outline"
            onClick={() =>
              executeProtectedAction(() => engine.startDemo(), "Run Project Automated Demo")
            }
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Panel title="Sun position" icon={<Sun className="h-4 w-4" />}>
              <div className="grid grid-cols-2 gap-2">
                ...
                <Metric
                  label="Day state"
                  value={s.night ? "NIGHT" : "DAY"}
                  tone={s.night ? "info" : "solar"}
                />
              </div>
            </Panel>
            ...
            <Panel title="Solar output" icon={<Zap className="h-4 w-4" />}>
              <div className="grid grid-cols-2 gap-2">
                ...
                <Metric
                  label="Today"
                  value={s.power.energyToday.toFixed(3)}
                  unit="kWh"
                  tone="success"
                />
                <Metric
                  label="Total"
                  value={s.power.energyTotal.toFixed(2)}
                  unit="kWh"
                  tone="success"
                />
