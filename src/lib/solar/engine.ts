...
class SolarEngine {
  ...
  private persist() {
    ...
    try {
      window.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ settings, panel, simMinutes, speed, mode, alerts: alerts.slice(0, 30), power }),
      );
    } catch {
  ...
  private tick() {
    ...
    if (s.emergencyStop) {
      ...
      const stepAz = Math.abs(dAz) > st.deadband ? Math.sign(dAz) * Math.min(6, Math.abs(dAz) / 30) : 0;
      const stepEl = Math.abs(dEl) > st.deadband ? Math.sign(dEl) * Math.min(6, Math.abs(dEl) / 30) : 0;
      target = {
    ...
    if (!s.emergencyStop) {
      ...
      motors = {
        azimuth:
          Math.abs(dAz) < 0.25 ? "IDLE" : moveAz > 0 ? "ROTATING RIGHT" : "ROTATING LEFT",
    ...
    const efficiency = night
      ? 0
      : Number(clamp(100 * Math.max(0, Math.cos((angular * Math.PI) / 180) ** 1.4), 0, 100).toFixed(1));
    ...
    const energyToday = Number(
      (simMinutes < s.simMinutes ? 0 : s.power.energyToday + (power * dtSimHours) / 1000).toFixed(3),
    );
    ...
    health["LDR Sensors"] = this.faultySensor ? "error" : irradiance < 0.12 && !night ? "warn" : "ok";
    ...
    // --- event driven alerts ---
    if (aligned && !this.wasAligned) this.pushAlert("success", "Solar panel successfully aligned with the Sun.");
    if (!aligned && this.wasAligned && !night) this.pushAlert("info", "Sun moved — adjusting panel position.");
    this.wasAligned = aligned;
    if (night && !s.night) this.pushAlert("info", "Night mode activated. Moving to parking position.");
    if (!night && s.night) this.pushAlert("info", "Sunrise detected. Solar tracking resumed.");
    if (!night && irradiance < 0.1 && s.irradiance >= 0.1) this.pushAlert("warning", "Low sunlight detected.");
    if (
      panel.azimuth > st.maxAzimuth - 5 &&
      s.panel.azimuth <= st.maxAzimuth - 5
    )
  ...
  pushAlert(level: AlertLevel, message: string) {
    const alerts = [
      { id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, level, message, at: Date.now() },
      ...this.state.alerts,
    ].slice(0, 40);
    this.set({ alerts });
  }
  ...
  setMode(mode: TrackingMode) {
    this.set({ mode });
    this.pushAlert("info", mode === "auto" ? "Automatic solar tracking started." : "Manual control enabled.");
  }
  ...
  emergencyStop() {
    this.set({ emergencyStop: true, simulating: false, motors: { azimuth: "STOPPED", elevation: "STOPPED" } });
    this.pushAlert("error", "EMERGENCY STOP engaged. All motors halted.");
  }

  resume() {
    this.set({ emergencyStop: false, simulating: true, motors: { azimuth: "IDLE", elevation: "IDLE" } });
  ...
  setPanel(next: Partial<Angles>) {
    ...
    const target = {
      ...
      elevation: clamp(next.elevation ?? this.state.target.elevation, st.minElevation, st.maxElevation),
    };
  ...
  resetPosition() {
    this.setPanel({ azimuth: this.state.settings.nightAzimuth, elevation: this.state.settings.nightElevation });
